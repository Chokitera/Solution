...
{
    public partial class Frmchamadoscliente : Form
    {
        private string id;
        private Form formularioAtual;
        Usuario perm = new Usuario();
        public Frmchamadoscliente(string id)
        {
        ...
        private void Frmchamadoscliente_Load(object sender, EventArgs e)
        {
            PreencherChamados();
        }
        ...
        public void PreencherChamados()
        {
            ...
            if (dt1 != null)
            {
                if (dt1.Rows.Count > 0)
                {
                    controls.ChamadoCliente[] chamados = new controls.ChamadoCliente[dt1.Rows.Count];

                    for (int i = 0; i < 1; i++)
                    {
                        foreach (DataRow row in dt1.Rows)
                        {
                            chamados[i] = new controls.ChamadoCliente();
            ...
            if (dt != null)
            {
                if(dt.Rows.Count > 0)
                {
                    controls.ChamadoCliente[] chamados = new controls.ChamadoCliente[dt.Rows.Count];
